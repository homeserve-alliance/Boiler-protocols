...
unsigned long OpenTherm::buildGetReturnTempRequest() {
  return buildRequest(OpenThermRequestType::READ, OpenThermMessageID::Tret, 0);
}

unsigned long OpenTherm::buildGetOEMDiagnosticCodeRequest() {
  return buildRequest(OpenThermRequestType::READ, 115, 0);//OpenThermMessageID::OEMDiagnosticCode, 0);
}

unsigned long OpenTherm::buildGetFlowRateRequest() {
  return buildRequest(OpenThermRequestType::READ, 19, 0);//OpenThermMessageID::OEMDiagnosticCode, 0);
}
...
float OpenTherm::getTemperature(unsigned long response) {
	float temperature = isValidResponse(response) ? ((response & 0xFFFF) / 256) : 0;
	return temperature;
}
...
float OpenTherm::getReturnTemp() {
  unsigned long response = sendRequest(buildGetReturnTempRequest());
  return getTemperature(response);
}

float OpenTherm::getFlowRate() {
  unsigned long response = sendRequest(buildGetFlowRateRequest());
  return getTemperature(response);
}

unsigned int OpenTherm::getdiagcode() {
  unsigned int response = sendRequest(buildGetOEMDiagnosticCodeRequest());
  return response;
}
